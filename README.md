...
    // Traduz o que o jogador digitar para o idioma 
    configurado
...
    // Exemplo: Mostrar um balão de fala traduzido acima 
    da cabeça do jogador
    SetPlayerChatBubble(playerid, text, 0xFFFFFFFF, 20.
    0, 5000);
    
    // Ou enviar no chat para todos
    new str[144], name[24];
    GetPlayerName(playerid, name, 24);
    format(str, sizeof(str), "[Traduzido] %s: %s", name, 
    text);
...
// Exemplo: Detectar idioma automaticamente e traduzir 
para Russo (ru)
...
```
#define EYK_TRANSLATE_LANG "en" // Muda o alvo padrão 
para Inglês
#include <EykTranslate>
```
### 📜 Licença
